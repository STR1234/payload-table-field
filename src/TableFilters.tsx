...
export const createDefaultFilterGroup = (columns: TableFieldColumnConfig[]): TableFilterGroup => ({
...
const matchesFilterCondition = <TRow extends Record<string, unknown>>(
  ...
  switch (condition.operator) {
    case 'contains':
      return normalizedRowValue.toLowerCase().includes(normalizedFilterValue.toLowerCase())
    case 'equals':
...
export const applyTableFilterGroups = <TRow extends Record<string, unknown>>(
  ...
  return rows.filter(row =>
    groups.some(group => group.and.every(condition => matchesFilterCondition(row, condition))),
  )
}
...
export function TableFilters({
  ...
  const operatorOptions: Array<{ label: string; value: TableFilterOperator }> = [
    ...
    { label: strings.operators.greaterThanOrEqual, value: 'greaterThanOrEqual' },
        ...
        return {
          ...
          and: group.and.map(condition => {
            ...
            return {
              ...
              ...nextCondition,
              value:
                nextCondition.operator && !operatorNeedsValue(nextCondition.operator)
                  ? ''
  ...
  if (groups.length === 0) {
    return (
      <div className="payload-table-field__filters-empty">
        <div className="payload-table-field__filters-label">{strings.noFiltersSet}</div>
        ...
        >
          <PlusIcon size={14} />
          <span className="payload-table-field__button-label">{strings.addFilter}</span>
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="payload-table-field__filters">
      <div className="payload-table-field__filters-label">{strings.filterWhere}</div>

      <div className="payload-table-field__filters-groups">
        {groups.map((group, groupIndex) => (
          <div className="payload-table-field__filter-group-wrap" key={group.id}>
            {groupIndex > 0 ? (
              <div className="payload-table-field__filter-connector">{strings.or}</div>
            ) : null}

            <div className="payload-table-field__filter-group">
              {group.and.map((condition, conditionIndex) => (
                <div className="payload-table-field__filter-row" key={condition.id}>
                  {conditionIndex > 0 ? (
                  ...
                  <div className="payload-table-field__filter-controls">
                    <select
                      ...
                      onChange={event =>
                        updateCondition(group.id, condition.id, { columnKey: event.target.value })
                      }
              ...
              <div className="payload-table-field__filter-actions">
                ...
                >
                  <PlusIcon size={14} />
                  <span className="payload-table-field__button-label">{strings.addCondition}</span>
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="payload-table-field__filter-actions">
        ...
        >
          <PlusIcon size={14} />
          <span className="payload-table-field__button-label">{strings.addOrGroup}</span>
        </button>
      </div>
    </div>
  )
